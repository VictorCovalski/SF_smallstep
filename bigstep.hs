...
bbigStep (Ig e1 e2,s )  = let 	(n1,s1) = abigStep (e1,s)
			 	(n2,s2) = abigStep (e2,s)
					in (n1 == n2, s)
bbigStep (And b1 b2,s )  = bbigStep (Ig e1 e2,s )  = let 	(n1,s1) = abigStep (e1,s)
			 	(n2,s2) = abigStep (e2,s)
					in case b1 of
					TRUE -> if b2 == TRUE then TRUE else FALSE
					FALSE -> FALSE

bbigStep (Ig b1 b2,s )  = let 	(n1,s1) = abigStep (b1,s)
			 	(b2,s2) = abigStep (b2,s)
					in (b1 == b2, s)
...
cbigStep (While b c, s) = let (b1,s1) = bbigStep (b,s)
				in case b1 of
					True -> let 	(_,s2) = cbigStep (c,s)
							...
							in (Skip,s3)
					False -> (Skip,s)
cbigStep(Try c1 c2,s) = let (c1,s1) = cbigStep(c1,s)
				in case c1 of
